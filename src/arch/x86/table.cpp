...
void idt_init() {
    ...
    for(unsigned int i=0x20;i<=0xFD;i++) {
    ...
    idt_structs[0xFE].offset = (size_t)&_isr_irq_fe;
	idt_structs[0xFE].type_attr = IDT_ATTR_PRESENT | IDT_ATTR_PRIV0 | IDT_INT_GATE_32;
	idt_structs[0xFE].selector = 0x08;
