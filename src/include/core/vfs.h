...
struct vfs_directory : public vfs_node {
    ...
    vfs_directory( vfs_node* p, vfs_fs *f, void* d, unsigned char* n ) : vfs_node(p, f, d, n) { this->type = vfs_node_types::directory; this->update = true; };
	vfs_directory(vfs_node* cp) : vfs_node( cp->parent, cp->fs, cp->fs_info, cp->name ) { this->type = vfs_node_types::directory; this->update = true; };
};
