...
#define kmalloc(l) std::malloc(l); std::cout << "+ malloc (size " << l << ") at " << __LINE__ << std::endl;
#define kfree(p) std::free(p); std::cout << "- free at " << __LINE__ << std::endl;
#else
#define kmalloc(l) std::malloc(l);
...
void *kmalloc(size_t length) {
    ...
    std::cout << "Memory allocation (size " << length << "): *" << std::hex << ptr << std::endl;
    return ptr;
}

void kfree(void* ptr) {
    std::cout << "Memory deallocation: *" << std::hex << ptr << std::endl;
    std::free(ptr);
}
*/

#endif
// __PRINTF_HOSTED_TESTING
...
int get_n_digits( int num, int base=10 ) {
    int i = 0;
    while(num > 0) {
        num /= base;
        i++;
...
char* itoa( signed long long int num, int base=10, bool add_space=false, bool add_plus=false, bool add_prefix=false ) {
...
int kvsnprintf( char* out, size_t bufsz, char* fmt, va_list args ) {
    ...
    while( (cur = *pos) != '\0' ) {
        if( cur == '%' ) {
            ...
            switch(specifier) {
                ...
                {
                    if(l) {
                        wint_t val1 = va_arg(args, wint_t);
                ...
                case 'p':
                {
                    ...
                    char *initial = itoa(val, 16, false, false, true);
...
int ksprintf(char *out, char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
...
int kvsprintf(char *out, char *fmt, va_list args) {
    int ret = kvsnprintf(out, 0, fmt, args);
    return ret;
}

int ksnprintf(char *out, size_t bufsz, char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
...
#ifndef __PRINTF_HOSTED_TESTING
...
void panic(char *str, ...) {
    ...
    if(panic_str != NULL) { // Don't panic WHILE panicking.
        ...
        char *o = kvsnprintf(buf, len+1, str, args2);
...
#ifdef __PRINTF_HOSTED_TESTING
int main() {
    //kprintf("Test: %u, %x, %c, %c\n", 100, 0xFFFF, 'a', 65);
    
    kprintf ("Characters:             %c %c \n", 'a', 65);
    kprintf ("Decimals:               %d %ld\n", 1977, 650000L);
    kprintf ("Preceding with blanks:  %10d \n", 1977);
    kprintf ("Preceding with zeros:   %010d \n", 1977);
    kprintf ("Some different radices: %d %x %o %#x %#o \n", 100, 100, 100, 100, 100);
    kprintf ("floats:                 %4.2f %+.0e %E \n", 3.1416, 3.1416, 3.1416);
    kprintf ("Width trick: %*d \n", 13, 10);
    kprintf ("%s \n", "A string");
#ifdef __PRINTF_HOSTED_TESTING_MEMTRACK
    list_leaks();
#endif
    return 0;
}
